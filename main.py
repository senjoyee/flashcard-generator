...
from flaschcard_generator import FlashcardGenerator
import logging
  ...
  if uploaded_file is not None:
      ...
      if st.button("Generate Flashcards"):
          with st.spinner("Generating flashcards..."):
              try:
                  result = generator.generate_flashcards(study_materials, uploaded_file.name)
                  logger.info("Flashcards generated successfully")

                  if "error" in result:
                      logger.error(f"Error in flashcard generation: {result['error']}")
                      st.error(f"Error generating flashcards: {result['error']}")
                  else:
                      st.success(f"Flashcards generated successfully! Saved to: {result['file_path']}")
